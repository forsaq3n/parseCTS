...
then
	...
	echo "this is 7.0 style report"
		...
		#list CTS version
		echo " "																								| tee -a $FILE
		grep -ir "suite_version=" "$1"  | sed 's/.*suite_version=/Suite version =/'  |   sed 's/\"//' |  sed 's/\">//'	|  sed 's/\".*//'| tee -a $FILE
		...
		telephonyLocation=$newNCTSReportDir"FeatureDeviceInfo.deviceinfo.json"	
		 telephony=$(sed ':a;N;$!ba;s/\",\n/ /g' $telephonyLocation | grep "android.hardware.telephony " |sed 's/.*name\": \"//' |  sed 's/telephony.*available\":/telephony :/' |sed 's/,//' )
...
#contaisn BYOD test
if grep -wqri "BYOD" "$2" ; then
   			echo $BYOD"			YES"												| tee -a $FILE
   		else
   			echo $BYOD"			NO"												| tee -a $FILE
fi

# Managed user feature present and storage in cts test
  if [ $newNCTSReport -ne 0 ]
  then
  	managedUserReport=$newNCTSReportDir"FeatureDeviceInfo.deviceinfo.json"	
	managedUsers=$(sed ':a;N;$!ba;s/\",\n/ /g' $managedUserReport | grep "android.software.managed_users " |sed 's/.*name\": \"//' |  sed 's/users.*available\":/users :/' |sed 's/,//' )
